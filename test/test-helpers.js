...
function makeExpectedJob(users, job, contacts = []) {
  const owner = users.find((user) => user.id === job.user_id);

  const number_of_contacts = contacts.filter(
    (contact) => contact.job_id === job.id
  ).length;
...
function seedJobsTables(db, users, jobs, contacts = []) {
  ...
  return db.transaction(async (trx) => {
    ...
    await trx.raw(`SELECT setval('jobs_id_seq', ?)`, [
